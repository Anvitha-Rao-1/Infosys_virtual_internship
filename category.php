...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_goal'])) {
    ...
    if ($title !== '') {
        $stmt = $pdo->prepare("INSERT INTO goals (user_id, category_id, title, description, frequency, target_per_week) VALUES (?,?,?,?,?,?)");
        $stmt->execute([$uid, $category['id'], $title, $desc, $freq, $target]);
    }
